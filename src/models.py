...
def load_model(MODEL_SCALE = 16, DROP_R = 0.5, version='trained', model_type='evac_plus', base_path=None):
    ...
    if model_type == 'evac':
        model = tf.keras.models.load_model(os.path.join(os.path.dirname(__file__), '../trained_model/evac/'), custom_objects={'dice_coef': dice_coef})
    elif model_type == 'evac_plus':
        model = tf.keras.models.load_model(os.path.join(os.path.dirname(__file__), '../trained_model/evac_plus/'), custom_objects={'comb_loss': comb_loss, 'dice_coef_orig': dice_coef_orig, 'dice_coef_real': dice_coef_real, 'dice_coef_comp': dice_coef_comp})
